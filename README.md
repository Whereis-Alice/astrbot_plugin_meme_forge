...
- 支持用户独立的持久化收藏夹：引用 Bot 生成的 meme 并回复 `/meme收藏`，之后可快速查回对应触发命令。
...
| `/meme工坊收藏` | 所有人 | 引用 Bot 生成的 meme 后收藏；别名为 `/meme收藏` |
| `/meme工坊收藏夹` | 所有人 | 查看自己的收藏和可直接复用的生成命令；别名为 `/meme收藏夹` |
| `/meme工坊取消收藏 <触发词>` | 所有人 | 删除一项收藏；别名为 `/meme取消收藏` |
...
所有命令都提供“Meme 工坊”命名空间，避免覆盖原插件的 `/meme帮助`、`/禁用meme` 等命令。如果其他插件占用了简短别名，请使用 `/meme工坊收藏`、`/meme工坊收藏夹` 等完整命令。
...
1. 找到 Bot 通过 Meme 工坊发送的表情包，普通生成和随机生成的都可以。
2. 引用这条图片消息，发送 `/meme收藏`。
3. 需要再次使用时发送 `/meme收藏夹`，复制其中给出的 `/meme <触发词>` 命令。

收藏夹按“平台 + 用户 ID”独立保存，不同用户互不影响，并且重启 AstrBot 后仍然保留。重复收藏同一个 meme 会将它移到收藏夹最前面；每个用户默认最多收藏 50 个，超过后移除最早的一项。可用 `/meme取消收藏 <触发词>` 删除指定收藏。

插件不会保存生成图片本身，只保存图片指纹、meme key 和触发词。引用收藏依赖插件生成时建立的指纹记录，因此升级到支持收藏的版本之前已经发送的旧图片无法追溯；图片被平台大幅裁剪或添加水印时也可能无法识别。
...
| `max_favorites` | `50` | 每个用户最多保存的收藏数量，超出后移除最早收藏 |
...
- 插件配置由 AstrBot 管理。
- 用户收藏、meme 生成图片指纹和对应 key 保存在 AstrBot 的插件隔离 KV 存储中；不会保存生成图片内容。
