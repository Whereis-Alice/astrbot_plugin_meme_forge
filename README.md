...
- 带透明区域的成图默认转成 GIF 再发送，绕开 QQ 手机端把 PNG 透明像素画成黑块的问题；透明 GIF 使用稳定的 255 号透明调色板索引，并让逻辑屏幕背景色指向同一透明槽位，不透明的图保持原格式。
...
`/meme提取`、`/提取meme`、`/meme更新检查` 和 `/meme自制列表`。为降低与其他插件的冲突，优先使用完整的“meme工坊”命名空间。PJSK 手写体表情走独立的 `/sk角色`、`/sk表情`、`/sk` 指令，用法见下方“PJSK 表情工坊（可选）”一节。
...
PJSK 表情工坊把 Project Sekai 的手写体贴纸做成「看图报号」的流程：先让 Bot 发一张带号的
总览图，你照着图报号，再把想写的话跟在后面。不需要背角色代号，也不需要记住某张图叫什么。
...
| 指令 | 权限 | 说明 |
|---|---|---|
| `/sk角色 [角色号或角色名]` | 所有人 | 不带参数出 26 人总览图；带角色号或角色名翻开单个角色页 |
| `/sk表情 [角色或表情序号]` | 所有人 | 同样能出总览图；给角色名或某张图的表情序号，则打开对应角色页 |
| `/sk <选择> <文字> [参数]` | 所有人 | 生成一张表情包 |
...
插件本体不包含任何 PJSK 图片或字体，仓库里也没有。只有管理员执行 `/sk素材安装 确认` 之后，
才会下载到插件的隔离数据目录：
...
- 左栏是 26 位角色（带主题色圆点、角色号和表情序号区间），支持按角色名或表情序号搜索，也可以直接「随便来一张」。
...
上游较新的 Release 会把文件名写成带构建工具链的形式，例如
`meme-emoji-linux-x86_64-1.93.1.so` 或 `meme-emoji-linux-x86_64-stable.so`，不再只有旧的
`meme-emoji-linux-x86_64.so`。Meme 工坊会按兼容工具链、`stable`、旧命名依次精确选择，安装时
仍会校验 SHA-256；不需要手动改名或上传文件。若日志仍显示只寻找无后缀文件名，请先更新到
v1.10.10 或更高版本并重载插件。
...
检查是只读操作，不会运行 `pip`、下载扩展或修改配置。`meme_emoji` 有更新时，按结果提示执行 `/meme工坊扩展更新 确认`，完成后重启 AstrBot。Gouqi 上游出现新提交时只会报告“未审阅”，不会下载或执行；需要先由 Meme 工坊更新适配版本。PJSK 素材缺失或不完整时只会提示 `/sk素材安装 确认`；素材固定在已审阅提交，存放于插件数据目录，不随插件版本更新。内置素材没有独立的只读版本接口，因此需要检查或补齐素材时仍执行 `/meme工坊资源检查`。
...
| `transparent_output_as_gif` | `true` | 含透明区域的输出图改用 GIF 发送，避免 QQ 手机端显示成黑块 |
...
如果 Bot 只回了一句「开始下载」之后就再没动静、数据目录里也是空的，那是 v1.10.0 的缺陷：
指令在发出第一条提示后就被提前结束，下载那一步根本没跑到。升级到 v1.10.1 或更高版本并重载
插件，再发一次 `/sk素材安装 确认` 即可 —— 现在开始下载、下载完成（开始校验解压）和最终结果
会各回一条消息，中途不会安静下来。

### `/sk 122 哟` 没反应，日志里写 `arguments do not match its definition`

v1.10.3 已修复，升级后重载插件即可。旧版本里 `/sk`、`/sk随机`、`/meme工坊自制新建` 只要带参数就会在
AstrBot 里抛 `TypeError: ... got an unexpected keyword argument 'args'`，聊天里则完全没有回复。原因是
插件自己的处理器签名和 AstrBot 的传参方式不兼容（它按关键字传参），和你的 AstrBot 版本无关，4.26.x 与
4.27.x 都会中招。
...
QQ 手机版不渲染 PNG 的 alpha 通道，透明背景会被填成一整块黑色，PJSK 贴纸这类整张镂空的图最明显；桌面端和 Web 端却是正常的，所以很容易被当成插件出图坏了。
v1.10.4 起插件默认把这类成图改成 GIF 再发送 —— GIF 用调色板里的一个透明色表达同样的镂空，手机端能正常显示。
本地渲染出来的透明 GIF（例如 Gouqi 和自制模板的动图）从 v1.10.8 起固定使用 255 号透明调色板索引；v1.10.9 又把逻辑屏幕背景色也对齐到同一个透明槽位，避免部分 QQ 客户端用普通背景色铺底。旧版本交给 Pillow 自动选索引，部分 QQ 客户端会把它当普通颜色，看起来就像透明通道丢了。引用图片作为输入时，如果适配器同时提供本地缓存路径和平台 URL，插件会优先读本地原件，URL 只作兜底，减少 QQ 转存时压平透明的机会。
不透明的图（照片类 meme、各种列表长图）不会被转换，所以不会因为 GIF 只有 256 色而掉画质。
半透明的边缘像素会按 alpha 128 取整成「全透明」或「全不透明」，这是 GIF 每帧只能标记一个透明色的格式限制。
想关掉就把配置里的 `transparent_output_as_gif` 设为 `false`。

### 透明图片用于「露露卡旋转」时背景变成空洞

这是 Gouqi 旋转模板的输入槽位与透明图片叠加方式导致的：模板的 PNG 在透明槽位里仍保存了
背景 RGB，旧版本只读取 alpha，透明输入就会把槽位也透出去。v1.10.10 起插件会在这两个旋转
模板的输入范围内恢复模板自带背景，再叠加透明主体；普通不透明图片和没有隐藏背景数据的模板
仍按原流程处理。升级并重载插件即可，不需要重新安装 Gouqi 素材。

### 扩展安装提示「最新 release 没有当前平台构建」

如果日志里只显示 `meme-emoji-linux-x86_64.so`，通常是旧版安装器还在寻找无后缀文件名。
`meme-emoji` 新版 Release 使用 `meme-emoji-linux-x86_64-1.93.1.so` 或 `-stable.so`；升级到
v1.10.10 或更高版本并重载插件后，安装器会按兼容工具链、`stable`、旧命名依次精确匹配。
它不会使用模糊前缀，也不会因此下载其他架构的库。确认安装时仍使用：
