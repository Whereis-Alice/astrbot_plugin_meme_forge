...
`meme_emoji` 与 Gouqi 扩展、自己动手做模板的 Meme Maker、看图选序号的 PJSK 手写体表情工坊，
以及用于查看、管理和创作的 AstrBot Dashboard 页面。
...
- 可选的 PJSK 表情工坊：先看总览图挑序号，再配文字出图，26 位角色、359 张底图全部本地渲染。
...
| `/meme工坊更新检查` | 管理员 | 只读检查内置引擎、扩展与 PJSK 素材状态 |
...
`/meme提取`、`/提取meme`、`/meme更新检查` 和 `/meme自制列表`。为降低与其他插件的冲突，优先使用完整的“meme工坊”命名空间。PJSK 手写体表情走独立的 `/pjsk表情`、`/pjsk` 指令，用法见下方“PJSK 表情工坊（可选）”一节。
...
PJSK 表情工坊把 Project Sekai 的手写体贴纸做成「看图选号」两步流程：先让 Bot 发一张带序号的
总览图，你照着图报序号，再把想写的话跟在后面。不需要背角色代号，也不需要记住某张图叫什么。

```text
/pjsk表情              → 26 位角色总览图（角色名 + 序号区间）
/pjsk表情 未来         → 初音未来全部姿势的缩略图（每张带序号）
/pjsk 206 Wonderhoy    → 出图
```
...
| 写法 | 示例 | 说明 |
|---|---|---|
| 全局序号 | `/pjsk 206 早上好` | 1 ~ 359，直接照总览图读 |
| 角色 + 编号 | `/pjsk 未来3 早上好` | 该角色的第 3 张，`miku3`、`MIKU 3` 同义 |
| 分开写 | `/pjsk 未来 3 早上好` | 与上一行等价 |
| 只写角色 | `/pjsk 未来 早上好` | 从该角色的图里随机一张 |
| 完全随机 | `/pjsk随机 早上好` | 从 359 张里随机一张 |
...
| `-x` | `-横` | 0 ~ 296 | 文字中心的横向位置 |
| `-y` | `-纵` | 0 ~ 256 | 文字中心的纵向位置 |
| `-r` | `-旋转` `-角度` | -10 ~ 10 | 旋转角度（度） |
| `-s` | `-字号` `-大小` | 10 ~ 100 | 字号 |
| `-l` | `-行距` `-spacing` | 0.8 ~ 3 | 行距倍数 |
| `-c` | `-弧形` `-弯曲` `-曲线` | 开关 | 弧形排版（沿弧线摆字） |
| `--scale` | `-倍数` `-清晰度` | 1 ~ 4 | 输出倍数，默认取配置项 |

```text
/pjsk 未来3 早上好 -s 34 -r 2 -c
/pjsk 206 今天也要\n加油 --scale 3
```

不写参数时使用每张图的官方默认排版；文字里的 `\n` 会换行，单次最多 6 行、160 字。
`-s 34` 和 `-s=34`、`--size 34` 都可以，全角数字也能识别。

### 指令

| 指令 | 权限 | 说明 |
|---|---|---|
| `/pjsk表情 [角色]` | 所有人 | 不带参数出角色总览图；带角色名出该角色的姿势缩略图 |
| `/pjsk <选择> <文字> [参数]` | 所有人 | 生成一张表情包 |
| `/pjsk随机 [文字] [参数]` | 所有人 | 随机一张，回复里带上它的序号和指令 |
| `/pjsk帮助` | 所有人 | 用法速查（不带任何参数的 `/pjsk` 也会显示） |
| `/pjsk素材状态` | 所有人 | 查看素材安装、校验和输出倍数 |
| `/pjsk素材安装 确认` | 管理员 | 下载并逐项校验素材，约 30 MB |

兼容别名：`/pjsk列表`、`/pjsk菜单`、`/pjsk角色`、`/随机pjsk`、`/pjsk抽一张`、`/pjsk用法` 等；
`/pjsk 表情`、`/pjsk 随机`、`/pjsk 帮助` 这种把子命令写在后面的形式也认。

### 素材与授权

插件本体不包含任何 PJSK 图片或字体，仓库里也没有。只有管理员执行 `/pjsk素材安装 确认` 之后，
才会下载到插件的隔离数据目录：
...
- 渲染全程在本机用 Pillow 完成，不调用任何外部绘图接口。
- MIT 许可覆盖的是这两个仓库的代码与文件组织。**角色形象与原作版权仍归 SEGA / Colorful Palette / Crypton Future Media 等权利人所有**，请按自己所在地区的规定在非商业场景下使用。
- 想彻底不用这个功能，把配置项 `pjsk_enabled` 关掉即可：指令不再注册，WebUI 的 PJSK 页显示提示，已下载的素材保留在数据目录里。

### 和其他功能的联动

- **收藏**：引用 Bot 发出的 PJSK 表情发送 `/meme收藏`，收藏夹里会给出 `/pjsk 206 你的文字` 形式的可复用命令；`/meme工坊取消收藏 pjsk 206` 移除。
...
五个标签页可用地址栏 `#overview` / `#library` / `#maker` / `#pjsk` / `#records` 直接进入：

- **总览**：表情库总量与启用比例、来源分布、`meme_emoji` 与 Gouqi 扩展状态、PJSK 素材状态、常用 meme 排行、活跃会话和最近活动时间线。卡片上的快捷入口可直接跳到对应标签并带上筛选条件。
- **表情库**：搜索名称与关键词，按标签、启用状态、来源筛选，并可按 key、图片数、文字数或来源排序；支持网格与列表两种密度。
- **工作台**：可视化编辑自制模板（`maker_enabled` 关闭时这里显示提示，标签徽标显示“关”）。
- **PJSK**：三栏式贴纸工作台，看着图选序号、调排版、实时预览（`pjsk_enabled` 关闭或素材未安装时显示提示，标签徽标显示“关”）。
...
- 左栏是 26 位角色（带主题色圆点和序号区间），支持按角色名或序号搜索，也可以直接「随便来一张」。
- 中栏上方是实时预览与序号翻页（`上一张` / `下一张`），下方是当前角色的贴纸缩略图网格，缩略图懒加载。
- 右栏填文字、调 x / y / 旋转 / 字号 / 行距 / 弧形 / 输出倍数，改完自动重渲染（约半秒防抖）；`用这张的官方原句` 一键填回默认文案，`恢复默认` 回到这张图的原始排版。
- 预览下方给出等价的聊天指令，点一下就能复制到聊天里发；预览图可以点开放大，也可以直接下载 PNG。
- 右栏底部列出素材来源、许可证、已审阅提交和安装时间，方便确认这台机器上装的是哪一版素材。
...
- PJSK 素材是否安装完整（本地底图数量对比清单），以及已审阅的底图提交、字体提交和它们的 MIT 许可。

检查是只读操作，不会运行 `pip`、下载扩展或修改配置。`meme_emoji` 有更新时，按结果提示执行 `/meme工坊扩展更新 确认`，完成后重启 AstrBot。Gouqi 上游出现新提交时只会报告“未审阅”，不会下载或执行；需要先由 Meme 工坊更新适配版本。PJSK 素材缺失或不完整时只会提示 `/pjsk素材安装 确认`；素材固定在已审阅提交，存放于插件数据目录，不随插件版本更新。内置素材没有独立的只读版本接口，因此需要检查或补齐素材时仍执行 `/meme工坊资源检查`。
...
| 配置项 | 默认值 | 说明 |
|---|---:|---|
| `pjsk_enabled` | `true` | 是否注册 `/pjsk` 系列指令并在 WebUI 显示「PJSK」页，重载后生效 |
...
- PJSK 底图、字体和自制模板素材保存在插件的隔离数据目录里；PJSK 表情每次都是即时渲染，不留成品图片。
- Dashboard 中的全局和会话记录面向拥有 AstrBot Dashboard 权限的管理员，请按部署环境的隐私要求配置权限。
- 更新检查会访问 PyPI、GitHub Release/Commit API；生成资源和扩展安装会访问相应上游 GitHub/CDN。Gouqi 安装只下载固定审阅提交的图片素材，PJSK 素材安装只在管理员确认后从两个 MIT 源仓库的固定提交下载底图与字体，两者都不会自动执行。网络图片和头像会访问其原始地址或腾讯头像 CDN，并受协议、超时和大小限制。
...
让管理员执行 `/pjsk素材安装 确认`。首次安装要从 GitHub 下载约 30 MB，网络慢时把配置项
`pjsk_download_timeout` 调大后重载插件。装完用 `/pjsk素材状态` 复核：底图应为 359/359，
字体一行显示已安装。校验不通过多半是下载被截断或代理返回了 HTML 页面，重跑一次安装即可，
它会覆盖不完整的文件。

### PJSK 表情的字太小或位置不对

每张底图都带官方默认排版，长句会自动缩小字号并折行。想手动调就用 `-s` 改字号、`-x` / `-y`
挪位置、`-c` 换成弧形排版；也可以在 WebUI 的 PJSK 页拖着调，满意后复制指令回聊天里用。
超过 6 行或 160 字的部分会被截断。
...
- [`camera-2018/astrbot_plugin_pjsk`](https://github.com/camera-2018/astrbot_plugin_pjsk)：PJSK 表情工坊的功能参考。其代码没有被引入——上游用 Playwright 驱动 Chromium 渲染（首次启动会自动安装浏览器与 Linux 系统依赖，可能需要 root 权限），且 README 声明 MIT 但仓库内没有 LICENSE 文件。本插件改成纯 Pillow 本地渲染，并把「先查 ID 再用 `-i` 传参」换成看总览图报序号的两步流程。
