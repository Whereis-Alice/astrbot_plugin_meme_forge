...
`/meme提取`、`/提取meme`、`/meme更新检查` 和 `/meme自制列表`。为降低与其他插件的冲突，优先使用完整的“meme工坊”命名空间。PJSK 手写体表情走独立的 `/sk表情`、`/sk` 指令，用法见下方“PJSK 表情工坊（可选）”一节。
...
```text
/sk表情              → 26 位角色总览图（角色名 + 序号区间）
/sk表情 未来         → 初音未来全部姿势的缩略图（每张带序号）
/sk 206 Wonderhoy    → 出图
```
...
| 指令 | 权限 | 说明 |
|---|---|---|
| `/sk表情 [角色]` | 所有人 | 不带参数出角色总览图；带角色名出该角色的姿势缩略图 |
...
兼容别名：`/sk列表`、`/sk菜单`、`/sk角色`、`/随机sk`、`/sk抽一张`、`/sk用法` 等；
`/sk 表情`、`/sk 随机`、`/sk 帮助` 这种把子命令写在后面的形式也认。

旧指令 `/pjsk`、`/pjsk表情`、`/pjsk随机`、`/pjsk帮助`、`/pjsk素材状态`、`/pjsk素材安装 确认`
