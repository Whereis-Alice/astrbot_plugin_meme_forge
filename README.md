...
- 带透明区域的成图默认转成 GIF 再发送，绕开 QQ 手机端把 PNG 透明像素画成黑块的问题；透明 GIF 使用稳定的 255 号透明调色板索引，不透明的图保持原格式。
...
本地渲染出来的透明 GIF（例如 Gouqi 和自制模板的动图）从 v1.10.8 起固定使用 255 号透明调色板索引；旧版本交给 Pillow 自动选索引，部分 QQ 客户端会把它当普通颜色，看起来就像透明通道丢了。引用图片作为输入时，如果适配器同时提供本地缓存路径和平台 URL，插件会优先读本地原件，URL 只作兜底，减少 QQ 转存时压平透明的机会。
