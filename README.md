...
重载插件后，在 AstrBot Dashboard 的插件页面打开 **Meme 工坊**。WebUI 使用紧凑侧边栏，避免把目录、详情和记录堆在一条长页面中；窄屏会自动切换为顶部导航。页面支持浅色和夜间主题，并分为：

- **概览**：表情库状态、扩展状态、常用 meme、活跃会话和最近活动；点击常用 meme 或会话可直接进入对应的管理视图。
- **表情库**：名称、关键词、标签和启用状态筛选，以及单个 meme 的参数、预览和素材管理。
- **最近记录**：全局最近成功生成记录，支持按会话筛选查看群友或私聊参与者的触发情况。
